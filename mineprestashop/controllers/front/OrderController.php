...
class OrderControllerCore extends FrontController
{
    ...
    public function initContent()
    {
        ...
        if (isset($_POST['taxInvoiceNeeded'])) {
            $taxInvoiceNeeded = $_POST['taxInvoiceNeeded'];
            error_log("Received taxInvoiceNeeded value: $taxInvoiceNeeded");
        }
    ...
    protected function buildCheckoutProcess(CheckoutSession $session, $translator)
    {
        ...
        $taxInvoiceNeeded = isset($_GET['taxInvoiceNeeded']) && $_GET['taxInvoiceNeeded'] === 'true';
        $setSelect = '';
        if ($taxInvoiceNeeded === true){
            $setSelect = 'true';
        }else if ($taxInvoiceNeeded === false) {
            $setSelect = 'false';
        }
        // echo $setSelect;

        if ($setSelect = 'true' or $setSelect === 'true') {
            $checkoutProcess->addStep(new CheckoutAddressesStep(
                $this->context,
                $translator,
