...
test('test', async ({ page }) => {
  // Test Case 1_1: login-buy-1-product
  // Open index
  await page.goto('http://localhost:8080/th/');

  // Test Case 1_1: Input data_login
  // Input data_login
  await page.click('a:has-text(" เข้าสู่ระบบ")');
  await page.fill('input#field-email', 'somsri111111@kkumail.com');
  await page.fill('input#field-password', '0889742987');
  await page.click('a:has-text(" เข้าสู่ระบบ")');
  await page.click('a:has-text(" หน้าหลัก")');
  //await page.click('a:has-text(" ซื้อสินค้าต่อ")');
